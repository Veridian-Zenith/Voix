...
| Binary executable + version | `./build/voix --version` | `Voix version 4.12.1 ...` |
...
5. **Verify binary**: `./build/voix --version` returns `v4.12.1` (version string must match `CMakeLists.txt` + `src/main.cpp` + `docs/voix.1` + `PKGBUILD` — sync them together, never individually).
