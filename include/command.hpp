...
private:
    ...
    void apply_environment(const SecurityProfile& profile,
                           const Config& config,
                           const CommandOptions& options,
                           const Rule& rule,
                           const UserIdentity& target,
    ...
    std::string resolve_absolute_command(std::string command, const Config& config);
    ...
    void do_exec(const std::string& cmd_str,
                 const std::vector<std::string>& args,
                 const CommandOptions& options,
                 const UserIdentity& target);
};
