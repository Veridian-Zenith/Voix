...
- [x] **Seccomp Allowlist Mode**: Default-deny `SCMP_ACT_KILL` profile (`seccomp_mode: allowlist`) with the 19 blacklisted syscalls permitted (`SCMP_ACT_ALLOW`). Profile-level opt-in; backward-compat `blacklist` default preserved. See `security.cpp` applySeccompAllowlist(), `security.hpp`, `THREATS.md`, `command.cpp` apply_kernel_confinement() (v4.12.0).
- [x] **Multi-arch Artifacts (aarch64/arm64)**: AUR `arch` expanded (`x86_64` → `x86_64 aarch64`) in both `voix` and `voix-bin` PKGBUILD (v4.12.0). `CMakeLists.txt` `VOIX_ARCH` knob documented. Native cross-compile (`clang --target=aarch64-unknown-linux-gnu`) requires target triple + architecture spec; `armv8-a` is not a native clang CPU name. Multi-arch CI matrix deferred to M-7 release pipeline.
